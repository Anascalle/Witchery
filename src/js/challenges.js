export const retos = {
    "Crea": [
        "Dibuja un objeto sin levantar el lápiz y haz que tu equipo lo adivine.",
        "Escribe una rima rápida usando la palabra 'misterio'.",
        "Haz una escultura con objetos cercanos en 30 segundos."
    ],
    "Actúa": [
        "Imita a un personaje famoso y haz que tu equipo lo adivine.",
        "Expresa una emoción sin hablar, solo con gestos.",
        "Recrea una escena de una película sin usar palabras."
    ],
    "Descifra": [
        "Resuelve este anagrama: 'EAGM'",
        "Encuentra la palabra oculta en este acertijo: 'Soy alto cuando soy joven, y corto cuando soy viejo'.",
        "Completa la serie numérica: 2, 4, 8, 16, __."
    ]
};
